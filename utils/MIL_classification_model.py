...
class MILModel_DSA(nn.Module):
    """
    Multiple Instance Learning (MIL) model, with a backbone classification model.
    The expected shape of input data is `[B, N, C, H, W]`,
    where `B` is the batch_size of PyTorch Dataloader and `N` is the number of images in the DSA sequence.
    ...
    def __init__(
        ...
        if backbone is None:
            ...
            if backbone_num_features is None:
                raise ValueError("Number of encoder features must be provided for a custom backbone model")
    ...
    def calc_weights(self, x: torch.Tensor) -> torch.Tensor:
        sh = x.shape

        if self.mil_mode == "att":
            a = self.attention(x)
            x = torch.softmax(a, dim=1)
            ...
            raise ValueError("Model mode without attention:" + str(self.mil_mode))
