...
from sklearn.metrics import classification_report, cohen_kappa_score, f1_score, precision_score, recall_score, accuracy_score, balanced_accuracy_score

from utils.MIL_classification_model import *
...
def train_epoch(model, loader, optimizer, scaler, epoch, args):
    ...
    for idx, batch_data in enumerate(loader):
        ...
        if args.rank == 0:
            print(
                "Epoch {}/{} {}/{}".format(epoch+1, args.epochs, idx+1, len(loader)),
                "loss: {:.4f}".format(loss),
                "acc: {:.4f}".format(acc),
...
def val_epoch(model, loader, epoch, args):
    ...
    with torch.no_grad():
        for idx, batch_data in enumerate(loader):
            ...
            with autocast(enabled=args.amp):
                if data.shape[1] > 1:
                    # During validation, we want to use all images
                    # and if its number is very big, we may run out of GPU memory
                    # in this case, we first iteratively go over subsets of images to calculate backbone features
                    # and at the very end calculate the classification output

                    logits = []
                    logits2 = []

                    for i in range(int(np.ceil(data.shape[1] / float(1)))):
                    ...
                    # if number of instances is one, we can run inference directly
                    logits = model(data)
            ...
            if args.rank == 0:
                print(
                    "Val epoch {}/{} {}/{}".format(epoch+1, args.epochs, idx+1, len(loader)),
                    "loss: {:.4f}".format(loss),
                    "acc: {:.4f}".format(acc),
                    "time {:.2f}s".format(time.time() - start_time),
                )
...
def test_epoch(model, loader, epoch, args):
    """One test epoch over the dataset"""
    ...
    with torch.no_grad():
        ...
        for idx, batch_data in enumerate(loader):
            ...
            with autocast(enabled=args.amp):
                if data.shape[1] > 1:
                    # During validation, we want to use all instances
                    # and if its number is very big, we may run out of GPU memory
                    # in this case, we first iteratively go over subsets of images to calculate backbone features
                    # and at the very end calculate the classification output
                    ...
                    for i in range(int(np.ceil(data.shape[1] / float(1)))):
                        data_slice = data[:, i : (i + 1)] # shape: [1,1,3,img_height,img_width] 
                        logits_slice = model(data_slice, no_head=True) # shape: [1 , 1, 2028]
                        logits.append(logits_slice)
                    ...
                    logits = torch.cat(logits, dim=1) # Converts logits list to tensor size [1, sequence_length, 2048] 
                    if has_extra_outputs:
                        extra_outputs["layer1"] = torch.cat([l[0] for l in logits2], dim=0)
                        extra_outputs["layer2"] = torch.cat([l[1] for l in logits2], dim=0)
                        extra_outputs["layer3"] = torch.cat([l[2] for l in logits2], dim=0)
                ...
                x = model.forward(data, no_head=True)
                weights = model.calc_weights(x)
            ...
            if args.rank == 0:
                print(
                    "Test epoch {}/{} {}/{}".format(epoch+1, args.epochs, idx+1, len(loader)),
                    "loss: {:.4f}".format(loss),
                    "acc: {:.4f}".format(acc),
                    "time {:.2f}s".format(time.time() - start_time),
                )
        ...
        PREDS = PREDS.get_buffer().cpu().numpy()
        TARGETS = TARGETS.get_buffer().cpu().numpy()
        WEIGHTS = WEIGHTS.get_buffer().cpu().numpy()
        
        qwk = cohen_kappa_score(TARGETS.astype(np.float64), PREDS.astype(np.float64), weights="quadratic")
        f1_score_test = f1_score(TARGETS.astype(np.float64),PREDS.astype(np.float64), average="weighted",zero_division=np.nan)
        precision_score_test = precision_score(TARGETS.astype(np.float64),PREDS.astype(np.float64), average="weighted",zero_division=np.nan)
        recall_score_test = recall_score(TARGETS.astype(np.float64),PREDS.astype(np.float64), average="weighted",zero_division=np.nan)
    return loss, acc, qwk, precision_score_test, recall_score_test, f1_score_test, TARGETS, PREDS, WEIGHTS
...
def load_MIL_datalist(
    data_list_file_path: PathLike,
    is_segmentation: bool = True,
    data_list_key: str = "training",
    base_dir: Optional[PathLike] = None,
):
    """Load the list of list of dicts where each dict is an image/label paths
    Args:
        data_list_file_path: the path to the json file of datalist.
        data_list_key: the key to get a list of dictionary to be used, default is "training". Others are "validation" and "test"
    Raises:
    ...
    Returns a list of list of data items, each of which is a dict keyed by element names, for example:
    .. code-block::
        [
            {'image': '/workspace/data/8998.01.jpg',  'label': 0},
            {'image': '/workspace/data/8998.01.jpg',  'label': 0}
        ]
    """
...
def main_worker(gpu, args):
    ...
    training_list = load_MIL_datalist(
        data_list_file_path=args.dataset_json,
        data_list_key="training",
    )
    validation_list = load_MIL_datalist(
        data_list_file_path=args.dataset_json,
        data_list_key="validation",
    )
    testing_list = load_MIL_datalist(
        data_list_file_path=args.dataset_json,
    ...
    train_transform = Compose(
        [
            ...
            # Data Augmentation
            Rand2DElasticd(keys="image", prob=0.6, spacing=30, magnitude_range=(0.1, 0.3)), #adds local deformations to the images.
            RandRotated(keys = "image",range_x=np.pi / 60, prob=0.2, keep_size=True),
            RandZoomd(keys = "image", min_zoom=0.8, max_zoom=1.5, prob=0.6),
            RandGaussianNoised(keys="image", prob=0.5, mean=0, std=0.01), # Introducing small random noise
    ...
    dataset_test = Dataset(data=testing_list, transform=test_transform)
    ...
    if args.rank == 0:
        print("Dataset training:", len(dataset_train), "validation:", len(dataset_valid), "testing", len(dataset_test))

    model = MILModel_DSA(num_classes=args.num_classes, pretrained=True, mil_mode=args.mil_mode)
    ...
    if args.test:
        # Use held-out test data on an existing checkpoint
        epoch_time = time.time()
	#loss, acc, qwk, precision_score_test, recall_score_test, f1_score_test
        test_loss, test_acc, test_qwk, test_precision, test_recall, test_f1, TARGETS, PREDS, WEIGHTS  = test_epoch(model, test_loader, epoch=0, args=args)
        if args.rank == 0:
            print(
                "Final testing loss: {:.4f}".format(test_loss),
                "acc: {:.4f}".format(test_acc),
                "qwk: {:.4f}".format(test_qwk),
                "Precision: {:.4f}".format(test_precision),
                "Recall: {:.4f}".format(test_recall),
                "F1 score: {:.4f}".format(test_f1),
                "time {:.2f}s".format(time.time() - epoch_time),
            )
            y_true = TARGETS.astype(np.float64)
            y_pred = PREDS.astype(np.float64)
            
            if args.label_crosswalk != None:
                df_label_crosswalk = pd.read_csv(args.label_crosswalk)
                class_labels = df_label_crosswalk.angio_run.to_list()
            else:
                class_labels = None
        ...
        if args.save_test_pickle:
            ...
            with open(pickle_filename, 'wb') as file:
                pickle.dump((PREDS, TARGETS, WEIGHTS), file)
    ...
    if args.mil_mode in ["att_trans", "att_trans_pyramid"]:
        ...
        params = [
            ...
            {"params": list(m.transformer.parameters()), "lr": 3e-05, "weight_decay": 0.1},
        ]
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        if args.rank == 0:
            print(
                "Final training  {}/{}".format(epoch + 1, n_epochs),
        ...
        if (epoch + 1) % args.val_every == 0:
            ...
            if args.rank == 0:
                print(
                    "Final validation  {}/{}".format(epoch+1, n_epochs),
                ...
                if val_acc > val_acc_max:
                    print("Accuracy ({:.6f} --> {:.6f})".format(val_acc_max, val_acc))
                    val_acc_max = val_acc
                    b_new_best = True
        ...
        if args.rank == 0 and args.logdir is not None:
            ...
            if b_new_best:
                print("Copying new best model to model_best.pt!!!!")
                shutil.copyfile(os.path.join(args.logdir, "model_final.pt"), os.path.join(args.logdir, "model_best.pt"))
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Multiple Instance Learning (MIL) DSA Sequence Classification.")
    ...
    parser.add_argument("--mil_mode", default="att_trans", help="MIL algorithm (mean,max,att,att_trans,att_trans_pyramid)")
    ...
    parser.add_argument("--save_test_pickle",
        action='store_true',
        help="Set TRUE to save a pickle of the TARGETS & PREDS to --logdir. Must specify logdir argument"
    )
    parser.add_argument("--label_crosswalk", default=None, help="Allows test function to display text labels. Provide path to label_crosswalk_DB_filename")
        
    parser.add_argument("--epochs", default=50, type=int, help="number of training epochs")
    parser.add_argument("--batch_size", default=4, type=int, help="batch size, the number of images per gpu")
...
if __name__ == "__main__":
    ...
    if args.dataset_json is None:
        # Error and Exit as a dataset must be provided
        print("A json dataset must be provided")
        exit(0)
    ...
    if args.distributed:
        ...
        print("Multigpu", ngpus_per_node, "learning rate", "{:.2E}".format(args.optim_lr))
